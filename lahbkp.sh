...
mkdir -p $SYSBKPDIR
mkdir -p $TOUPLOAD
rsync -rRL --files-from=$APPDIR/files.list --exclude-from=$APPDIR/excludes.list /  $SYSBKPDIR
...
## 4. Dump LDAP
slapcat -n 0 -l $SYSBKPDIR/ldap_config.ldif
slapcat > $SYSBKPDIR/ldap_dump.ldif


## 5. Archive all and split by volumes
tar chjf $TOUPLOAD/$NOW.tar.bz2 /var/www/wiki
split -b 500M $TOUPLOAD/$NOW.tar.bz2 $TOUPLOAD/$NOW.tar.bz2_part_ -a 4

## 6. Send backups to TG
cd $APPDIR
